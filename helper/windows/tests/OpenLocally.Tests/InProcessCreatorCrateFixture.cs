...
/// <summary>In-process loopback contract fixture for ordinary test execution; it has no manual wrapper dependency.</summary>
internal sealed class InProcessCreatorCrateFixture : IAsyncDisposable
{
    ...
    private InProcessCreatorCrateFixture(TcpListener listener)
    {
        _listener = listener;
        _serveTask = ServeAsync();
    }

    internal Uri Origin => new($"http://127.0.0.1:{((IPEndPoint)_listener.LocalEndpoint).Port}/");
    internal int RequestCount => Volatile.Read(ref _requests);
    internal List<InProcessFixtureRequest> Requests { get; } = [];

    internal static Task<InProcessCreatorCrateFixture> StartAsync()
    {
        var listener = new TcpListener(IPAddress.Loopback, 0);
        listener.Start();
        return Task.FromResult(new InProcessCreatorCrateFixture(listener));
    }
    ...
    private async Task HandleAsync(TcpClient client)
    {
        ...
        await using (NetworkStream stream = client.GetStream())
        {
            try
            {
                InProcessFixtureHttpRequest? request = await ReadRequestAsync(stream, _lifetime.Token).ConfigureAwait(false);
                ...
                var recorded = new InProcessFixtureRequest(request.Method, request.PathAndQuery, hasBearer, tokenInUrl, request.Body);
                lock (Requests) Requests.Add(recorded);
                Interlocked.Increment(ref _requests);
                await WriteResponseAsync(stream, recorded, _lifetime.Token).ConfigureAwait(false);
            }
            catch (OperationCanceledException) when (_lifetime.IsCancellationRequested) { }
            catch (IOException) { }
        }
    ...
    private static async Task WriteResponseAsync(NetworkStream stream, InProcessFixtureRequest request, CancellationToken cancellationToken)
    {
        int status = 200;
        byte[] body;
        string contentType;
        ...
        else if (request.HasBearerAuthorization && request.Method == "GET" && request.PathAndQuery == $"/social-prep/{SessionId}/assets/1")
        {
            body = Encoding.UTF8.GetBytes("creatorcrate-in-process-media");
    ...
    private static async Task<InProcessFixtureHttpRequest?> ReadRequestAsync(NetworkStream stream, CancellationToken cancellationToken)
    {
        ...
        return new InProcessFixtureHttpRequest(requestLine[0], requestLine[1], values, Encoding.UTF8.GetString(body));
    }
    ...
    private sealed record InProcessFixtureHttpRequest(string Method, string PathAndQuery, IReadOnlyDictionary<string, string> Headers, string Body);
}

internal sealed record InProcessFixtureRequest(string Method, string PathAndQuery, bool HasBearerAuthorization, bool TokenInUrl, string Body)
{
        ...
        get
        {
            try
            {
                ...
                return document.RootElement.ValueKind == JsonValueKind.Object && document.RootElement.EnumerateObject().Count() == 1 &&
                    ...
                    intent.GetString() == InProcessCreatorCrateFixture.MediaToken;
            }
