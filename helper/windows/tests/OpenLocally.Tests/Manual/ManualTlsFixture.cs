...
/// <summary>Opt-in self-signed loopback listener for proving normal TLS validation remains enabled.</summary>
internal sealed class ManualTlsFixture : IAsyncDisposable
{
    ...
    private ManualTlsFixture(TcpListener listener, X509Certificate2 certificate)
    {
        _listener = listener;
    ...
    internal static Task<ManualTlsFixture> StartAsync()
    {
        ...
        listener.Start();
        return Task.FromResult(new ManualTlsFixture(listener, certificate));
    }
