...
describe('Phase 6B HTTP workflow', () => {
  ...
  describe('release list — readiness filters', () => {
    ...
    it('publishable filter shows only ready releases with present selected assets', async () => {
      ...
      await makeReadyRelease(projectId, 'HTTP Blocked Missing', { present: false, missing: true });
       const plannedProjectId = await createProject(app, { title: 'HTTP Planned Project', status: 'planned' });
       await createRelease(app, { projectId: plannedProjectId, title: 'HTTP Planned', status: 'planned' });
    ...
    it('list and board views agree on filtered release set', async () => {
      ...
      const blocked = await makeReadyRelease(projectId, 'Consistency Blocked', { present: false });
       const nonReadyProjectId = await createProject(app, { title: 'Consistency Non-Ready Project', status: 'tbd' });
       await createRelease(app, { projectId: nonReadyProjectId, title: 'Consistency Idea', status: 'tbd' });
  ...
  describe('release calendar', () => {
    ...
    it('does not show release readiness or asset fields on calendar entries', async () => {
      ...
      await createRelease(app, {
        projectId,
        title: 'Calendar No Readiness Release',
        plannedDate: '2025-06-15',
      });

      const res = await app.testAgent.get('/calendar?month=2025-06').expect(200);
      expect(res.text).toContain('Calendar No Readiness Release');
  ...
  describe('cross-view publish consistency', () => {
    ...
    it('keeps publishable and blocked readiness consistent across every planning view and publish endpoint', async () => {
      ...
      const publishableDetail = await app.testAgent.get(`/releases/${publishableId}`).expect(200);
      ...
      const blockedDetail = await app.testAgent.get(`/releases/${blockedId}`).expect(200);
      ...
      const dashboard = await app.testAgent.get('/').expect(200);
      ...
      // The release-backed calendar does not surface per-release readiness;
      // this view still must render normally alongside the other release
      // views for the same project.
      const calendar = await app.testAgent.get('/calendar?month=2025-06').expect(200);
      ...
      const blockedAfterDetail = await app.testAgent.get(`/releases/${blockedId}`).expect(200);
      ...
      const publishableResponse = await app.testAgent
